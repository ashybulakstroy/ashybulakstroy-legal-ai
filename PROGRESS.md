...
- Добавлен эндпоинт `GET /api/v1/autocomplete?q=...` (routes.py)
- Поиск подсказок по: названиям законов, заголовкам статей, тексту статей
- **Поиск:** выпадающий список под полем ввода с навигацией клавишами, debounce 250ms, подсветка совпадений
- **Консультация:** автодополнение по последнему слову, debounce 400ms, открытие закона в новой вкладке

### Улучшение релевантности поиска (legislation_service.py)
- Динамический порог: законы ниже 40% от макс скора отсекаются
- Штраф за отсутствие совпадения в заголовке: закон без совпадения в названии или заголовках статей отсекается
- Вес токена по длине: длинные токены (специфичные) имеют больший вес
- Дифференцированные веса: название закона ×15, заголовок статьи ×10, категория ×5, текст статьи ×1
- Фильтр common_prefix_score >= 7 для заголовков статей (убирает ложные срабатывания на слово «получить»)
